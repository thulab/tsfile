...
import com.corp.delta.tsfile.filter.utils.IntInterval;
import com.corp.delta.tsfile.filter.utils.Interval;
import com.corp.delta.tsfile.filter.visitorImpl.ConvertExpressionVisitor;
...
public class IntFilterVerifier extends FilterVerifier implements FilterVisitor<IntInterval> {
	...
	public <T extends Comparable<T>> IntInterval visit(LtEq<T> ltEq) {
		...
		if(ans.v[1] == Integer.MIN_VALUE && ans.flag[1] == false)
			ans.flag[0] = false;
		else 
			ans.flag[0] = true;
		ans.v[0] = Integer.MIN_VALUE;
	...
	public <T extends Comparable<T>> IntInterval visit(GtEq<T> gtEq) {
		...
		if(ans.v[0] == Integer.MAX_VALUE && ans.flag[0] == false)
			ans.flag[1] = false;
		else 
			ans.flag[1] = true;
	...
	public IntInterval intersection(IntInterval left, IntInterval right) {
		IntInterval ans = new IntInterval();
	...
	public IntInterval union(IntInterval left, IntInterval right) {	 
		int l = 0, r = 0;
