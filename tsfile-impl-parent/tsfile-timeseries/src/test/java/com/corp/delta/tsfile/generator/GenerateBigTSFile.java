package com.corp.delta.tsfile.generator;
...
public class GenerateBigTSFile {
    ...
    private static void writeToFile(long spaceLimit) throws InterruptedException, IOException {
        ...
        while (currentSpace < spaceLimit) {
            if (lineCount % 1000000 == 0) {
                ...
                currentSpace =
                        (long) FileUtils.getLocalFileByte(outputDataFile, Unit.B)
                                + innerWriter.updateMemSizeForAllGroup();
                LOG.info("write line:{},use time:{}s, space:{}", lineCount,
                        (endTime - startTime) / 1000,
                        FileUtils.transformUnit(currentSpace, Unit.MB));
    ...
    public static void main(String[] args) throws IOException, InterruptedException, WriteProcessException {
        ...
        long size =
                (long) FileUtils
                        .transformUnitToByte(Double.valueOf(args[1]), Unit.valueOf(args[2]));
        if (args.length >= 4)
            setRowGroupSize =
