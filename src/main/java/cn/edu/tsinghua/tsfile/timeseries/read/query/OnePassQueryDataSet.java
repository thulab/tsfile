...
public class OnePassQueryDataSet implements QueryDataSet {
  ...
  public void initForRecord() {
    ...
    for (String key : mapRet.keySet()) {
      ...
      if (cols[i] != null
          && (cols[i].valueLength > 0 || cols[i].timeLength > 0 || cols[i].emptyTimeLength > 0)) {
  ...
  public OldRowRecord getNextRecord() {
    ...
    for (int i = 0; i < size; i++) {
      ...
      } else if (emptyTimeIdxs[i] < cols[i].emptyTimeLength
          && minTime == cols[i].getEmptyTime(emptyTimeIdxs[i])) {
        field.setNull(true);
        emptyTimeIdxs[i]++;
        long nextTime = Long.MAX_VALUE;
        if (emptyTimeIdxs[i] < cols[i].emptyTimeLength) {
        ...
        switch (field.dataType) {
          ...
          default:
            throw new UnSupportedDataTypeException(
                "UnSupported datatype: " + String.valueOf(field.dataType));
        }
