...
public class MergeQueryDataSet implements QueryDataSet {
  ...
  public MergeQueryDataSet(LinkedHashMap<Path, SeriesReader> readersOfSelectedSeries)
      throws IOException {
  ...
  public RowRecord next() throws IOException {
    ...
    while (heap.size() > 0 && heap.peek().timestamp == aimPoint.timestamp) {
      ...
      if (readersOfSelectedSeries.get(point.path).hasNext()) {
        ...
        heap.add(
            new Point(point.path, nextTimeValuePair.getTimestamp(), nextTimeValuePair.getValue()));
      }
    }
    return rowRecord;
  }
