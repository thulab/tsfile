...
public class HadoopQueryEngine extends QueryEngine {
  ...
  public HadoopQueryEngine(ITsRandomAccessFileReader raf,
      List<RowGroupMetaData> rowGroupMetaDataList) throws IOException {
  ...
  public OnePassQueryDataSet queryWithSpecificRowGroups(List<String> deviceIdList,
      List<String> sensorIdList, FilterExpression timeFilter, FilterExpression freqFilter,
      FilterExpression valueFilter) throws IOException {
    if (deviceIdList == null)
      deviceIdList = initDeviceIdList();
    if (sensorIdList == null)
      sensorIdList = initSensorIdList();
  ...
  private OnePassQueryDataSet queryWithoutFilter(List<Path> paths) throws IOException {
    return new IteratorOnePassQueryDataSet(paths) {
      ...
      public DynamicOneColumnData getMoreRecordsForOneColumn(Path p, DynamicOneColumnData res)
          throws IOException {
        return recordReader.getValueInOneColumnWithoutException(res, FETCH_SIZE,
            p.getDeltaObjectToString(), p.getMeasurementToString());
      }
    };
  }
