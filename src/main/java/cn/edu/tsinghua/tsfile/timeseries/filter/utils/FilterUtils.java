...
public class FilterUtils {
  ...
  public static SingleSeriesFilterExpression construct(String deltaObject, String measurement,
      String filterType, String exp, RecordReader recordReader) throws IOException {
    ...
    if (exp.charAt(0) != '(') {
      ...
      if (exp.charAt(0) == '=') {
        if (type == 0) {
          ...
          return FilterFactory.eq(FilterFactory.longFilterSeries(deltaObject, measurement,
              FilterSeriesType.TIME_FILTER), v);
        } else if (type == 1) {
          float v = Float.valueOf(exp.substring(offset, exp.length()).trim());
          return FilterFactory.eq(FilterFactory.floatFilterSeries(deltaObject, measurement,
              FilterSeriesType.FREQUENCY_FILTER), v);
        } else {
          if (recordReader == null) {
            int v = Integer.valueOf(exp.substring(offset, exp.length()).trim());
            return FilterFactory.eq(FilterFactory.intFilterSeries(deltaObject, measurement,
                FilterSeriesType.VALUE_FILTER), v);
          }
          ...
          if (col instanceof IntFilterSeries) {
            ...
            return FilterFactory.eq(FilterFactory.booleanFilterSeries(deltaObject, measurement,
                FilterSeriesType.VALUE_FILTER), v);
            ...
            return FilterFactory.eq(FilterFactory.doubleFilterSeries(deltaObject, measurement,
                FilterSeriesType.VALUE_FILTER), v);
          } else if (col instanceof StringFilterSeries) {
            String v = String.valueOf(exp.substring(offset, exp.length()).trim());
            return FilterFactory.eq(FilterFactory.stringFilterSeries(deltaObject, measurement,
                FilterSeriesType.VALUE_FILTER), new Binary(v));
          } else {
        ...
        if (type == 0) {
          long v = Long.valueOf(exp.substring(offset, exp.length()).trim());
          return FilterFactory.gtEq(FilterFactory.longFilterSeries(deltaObject, measurement,
              FilterSeriesType.TIME_FILTER), v, ifEq);
        } else if (type == 1) {
          float v = Float.valueOf(exp.substring(offset, exp.length()).trim());
          return FilterFactory.gtEq(FilterFactory.floatFilterSeries(deltaObject, measurement,
              FilterSeriesType.FREQUENCY_FILTER), v, ifEq);
        } else {
          if (recordReader == null) {
            int v = Integer.valueOf(exp.substring(offset, exp.length()).trim());
            return FilterFactory.gtEq(FilterFactory.intFilterSeries(deltaObject, measurement,
                FilterSeriesType.VALUE_FILTER), v, ifEq);
          }
          FilterSeries<?> col = recordReader.getColumnByMeasurementName(deltaObject, measurement);
          if (col instanceof IntFilterSeries) {
            int v = Integer.valueOf(exp.substring(offset, exp.length()).trim());
            ...
            double v = Double.valueOf(exp.substring(offset, exp.length()).trim());
            return FilterFactory.gtEq(FilterFactory.doubleFilterSeries(deltaObject, measurement,
                FilterSeriesType.VALUE_FILTER), v, ifEq);
          } else if (col instanceof StringFilterSeries) {
            String v = String.valueOf(exp.substring(offset, exp.length()).trim());
            return FilterFactory.gtEq(FilterFactory.stringFilterSeries(deltaObject, measurement,
                FilterSeriesType.VALUE_FILTER), new Binary(v), ifEq);
          } else {
            throw new UnSupportedDataTypeException("Construct FilterSeries: " + col);
          }
        ...
        if (type == 0) {
          long v = Long.valueOf(exp.substring(offset, exp.length()).trim());
          return FilterFactory.ltEq(FilterFactory.longFilterSeries(deltaObject, measurement,
              FilterSeriesType.TIME_FILTER), v, ifEq);
        } else if (type == 1) {
          float v = Float.valueOf(exp.substring(offset, exp.length()).trim());
          return FilterFactory.ltEq(FilterFactory.floatFilterSeries(deltaObject, measurement,
              FilterSeriesType.FREQUENCY_FILTER), v, ifEq);
        } else {
          // default filter
          if (recordReader == null) {
            int v = Integer.valueOf(exp.substring(offset, exp.length()).trim());
            return FilterFactory.ltEq(FilterFactory.intFilterSeries(deltaObject, measurement,
                FilterSeriesType.VALUE_FILTER), v, ifEq);
          }
          FilterSeries<?> col = recordReader.getColumnByMeasurementName(deltaObject, measurement);
          if (col instanceof IntFilterSeries) {
            ...
            double v = Double.valueOf(exp.substring(offset, exp.length()).trim());
            return FilterFactory.ltEq(FilterFactory.doubleFilterSeries(deltaObject, measurement,
                FilterSeriesType.VALUE_FILTER), v, ifEq);
          } else if (col instanceof StringFilterSeries) {
            String v = String.valueOf(exp.substring(offset, exp.length()).trim());
            return FilterFactory.ltEq(FilterFactory.stringFilterSeries(deltaObject, measurement,
                FilterSeriesType.VALUE_FILTER), new Binary(v), ifEq);
          } else {
            throw new UnSupportedDataTypeException("Construct FilterSeries: " + col);
          }
        ...
        // long v = Long.valueOf(exp.substring(offset,exp.length()).trim());
        // return FilterFactory.ltEq(FilterFactory.longColumn(deltaObject, measurement, ifTime), v,
        // ifEq);
      }
      return null;
    }
    ...
    if (!texp.equals("")) {
      filters.add(construct(deltaObject, measurement, filterType,
          texp.substring(1, texp.length() - 1), recordReader));
    }
    ...
    for (int i = 0; i < exp.length(); i++) {
      ...
      if (numbraket == 0 && operator) {
        ...
        FilterExpression filter =
            constructCrossFilter(texp.substring(1, texp.length() - 1), recordReader);
        filters.add(filter);
        operators.add(c);
