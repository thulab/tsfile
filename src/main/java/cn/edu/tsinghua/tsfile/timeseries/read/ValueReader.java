...
/**
 * @author Jinrui Zhang This class is mainly used to read one column of data in RowGroup. It
 *         provides a number of different methods to read data in different ways.
 */
public class ValueReader {
  ...
  public ValueReader(long offset, long totalSize, TSDataType dataType, TsDigest digest) {
    Encoding timeEncoding =
        getEncodingByString(TSFileDescriptor.getInstance().getConfig().timeSeriesEncoder);
  ...
  public ValueReader(long offset, long totalSize, TSDataType dataType, TsDigest digest,
      ITsRandomAccessFileReader raf, CompressionTypeName compressionTypeName) {
   ...
   */
  public ValueReader(long offset, long totalSize, TSDataType dataType, TsDigest digest,
      ITsRandomAccessFileReader raf, List<String> enumValues,
  ...
  /**
   * //TODO what about timeFilters? Judge whether current column is satisfied for given filters
   */
  public boolean columnSatisfied(SingleSeriesFilterExpression valueFilter,
      SingleSeriesFilterExpression freqFilter, SingleSeriesFilterExpression timeFilter) {
    ...
    if (valueFilter != null) {
      ...
      if (getDataType() == TSDataType.ENUMS) {
        String minString = enumValues.get(
            BytesUtils.bytesToInt(digest.getStatistics().get(StatisticConstant.MIN_VALUE).array())
                - 1);
        String maxString = enumValues.get(
            BytesUtils.bytesToInt(digest.getStatistics().get(StatisticConstant.MAX_VALUE).array())
                - 1);
        valueDigest = new DigestForFilter(ByteBuffer.wrap(BytesUtils.StringToBytes(minString)),
            ByteBuffer.wrap(BytesUtils.StringToBytes(maxString)), TSDataType.TEXT);
      } else {
        valueDigest = new DigestForFilter(digest.getStatistics().get(StatisticConstant.MIN_VALUE),
            digest.getStatistics().get(StatisticConstant.MAX_VALUE), getDataType());
      }
    }

    DigestVisitor valueVisitor = new DigestVisitor();
    IntervalTimeVisitor timeVisitor = new IntervalTimeVisitor();
    if (valueVisitor.satisfy(valueDigest, valueFilter)
        && timeVisitor.satisfy(timeFilter, startTime, endTime)) {
      LOG.debug(String.format(
          "current series is satisfy the time filter and value filter, start time : %s, end time : %s",
          startTime, endTime));
      return true;
    }
    return false;
  ...
  /**
   * Judge whether current page is satisfied for given filters according to the digests of this page
   */
  public boolean pageSatisfied(DigestForFilter timeDigestFF, DigestForFilter valueDigestFF,
    ...
    if ((valueFilter == null && timeFilter == null)
        || (valueFilter != null
            && (valueDigestFF == null || digestVisitor.satisfy(valueDigestFF, valueFilter)))
   ...
   */
  public DynamicOneColumnData readOneColumn(DynamicOneColumnData res, int fetchSize)
      throws IOException {
    return readOneColumnUseFilter(res, fetchSize, null, null, null);
  }

  public SingleValueVisitor<?> getSingleValueVisitorByDataType(TSDataType type,
      SingleSeriesFilterExpression filter) {
    switch (type) {
      case INT32:
  ...
  public DynamicOneColumnData readOneColumnUseFilter(DynamicOneColumnData res, int fetchSize,
    ...
    if (columnSatisfied(valueFilter, freqFilter, timeFilter)) {
      ...
      while ((res.pageOffset - fileOffset) < totalSize
          && (res.valueLength - currentLength) < fetchSize) {
        ...
        if (pageDigest != null) {
          if (getDataType() == TSDataType.ENUMS) {
            String minString = enumValues.get(BytesUtils.bytesToInt(
                pageDigest.getStatistics().get(StatisticConstant.MIN_VALUE).array()) - 1);
            String maxString = enumValues.get(BytesUtils.bytesToInt(
                pageDigest.getStatistics().get(StatisticConstant.MAX_VALUE).array()) - 1);
            valueDigestFF =
                new DigestForFilter(ByteBuffer.wrap(BytesUtils.StringToBytes(minString)),
                    ByteBuffer.wrap(BytesUtils.StringToBytes(maxString)), TSDataType.TEXT);
          } else {
            valueDigestFF =
                new DigestForFilter(pageDigest.getStatistics().get(StatisticConstant.MIN_VALUE),
                    pageDigest.getStatistics().get(StatisticConstant.MAX_VALUE), getDataType());
          }
        }
        ...
        if (pageSatisfied(timeDigestFF, valueDigestFF, timeFilter, valueFilter, freqFilter)) {
          ...
          setDecoder(Decoder.getDecoderByType(pageHeader.getData_page_header().getEncoding(),
              getDataType()));
  ...
  /**
   * Read time-value pairs whose time is be included in timeRet. WARNING: this function is only for
   * "time" Series
   *
        ...
        switch (dataType) {
          case BOOLEAN:
            while (i < timeValues.length && timeIdx < timestamps.length) {
              ...
              while (timeIdx < timestamps.length && i < timeValues.length
                  && timestamps[timeIdx] < timeValues[i]) {
                timeIdx++;
              }
            }
            break;
