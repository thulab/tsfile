...
public class QueryFilterOptimizer {
  ...
  public QueryFilter convertGlobalTimeFilter(QueryFilter queryFilter, List<Path> selectedSeries)
    ...
    if (queryFilter instanceof UnaryQueryFilter) {
      ...
      if (left.getType() == QueryFilterType.GLOBAL_TIME
          && right.getType() == QueryFilterType.GLOBAL_TIME) {
        return combineTwoGlobalTimeFilter((GlobalTimeFilter) left, (GlobalTimeFilter) right,
            queryFilter.getType());
      } else if (left.getType() == QueryFilterType.GLOBAL_TIME
          && right.getType() != QueryFilterType.GLOBAL_TIME) {
        ...
        if (midRet.getLeft().getType() == QueryFilterType.GLOBAL_TIME
            || midRet.getRight().getType() == QueryFilterType.GLOBAL_TIME) {
          return convertGlobalTimeFilter(midRet, selectedSeries);
        } else {
          return midRet;
        }

      } else if (left.getType() == QueryFilterType.SERIES
          && right.getType() == QueryFilterType.SERIES) {
        return queryFilter;
      }
    }
    throw new UnsupportedOperationException(
        "unknown queryFilter type: " + queryFilter.getClass().getName());
  }

  private QueryFilter handleOneGlobalTimeFilter(GlobalTimeFilter globalTimeFilter,
      QueryFilter queryFilter, List<Path> selectedSeries, QueryFilterType relation)
      throws QueryFilterOptimizationException {
    ...
    if (regularRightQueryFilter instanceof GlobalTimeFilter) {
      return combineTwoGlobalTimeFilter(globalTimeFilter,
          (GlobalTimeFilter) regularRightQueryFilter, relation);
    }
    if (relation == QueryFilterType.AND) {
      addTimeFilterToQueryFilter((globalTimeFilter).getFilter(), regularRightQueryFilter);
      return regularRightQueryFilter;
    } else if (relation == QueryFilterType.OR) {
      return QueryFilterFactory.or(
          convertGlobalTimeFilterToQueryFilterBySeriesList(globalTimeFilter, selectedSeries),
          queryFilter);
    }
    throw new QueryFilterOptimizationException("unknown relation in queryFilter:" + relation);
  }

  private QueryFilter convertGlobalTimeFilterToQueryFilterBySeriesList(GlobalTimeFilter timeFilter,
      List<Path> selectedSeries) throws QueryFilterOptimizationException {
    if (selectedSeries.size() == 0) {
      throw new QueryFilterOptimizationException("size of selectSeries could not be 0");
    }
    SeriesFilter firstSeriesFilter =
        new SeriesFilter(selectedSeries.get(0), timeFilter.getFilter());
    QueryFilter queryFilter = firstSeriesFilter;
    for (int i = 1; i < selectedSeries.size(); i++) {
      queryFilter = QueryFilterFactory.or(queryFilter,
          new SeriesFilter(selectedSeries.get(i), timeFilter.getFilter()));
    }
    return queryFilter;
  }
  ...
  private void addTimeFilterToQueryFilter(Filter<?> timeFilter, QueryFilter queryFilter) {
    if (queryFilter instanceof SeriesFilter) {
      ...
      throw new UnsupportedOperationException(
          "queryFilter should contains only SeriesFilter but other type is found:"
  ...
  private GlobalTimeFilter combineTwoGlobalTimeFilter(GlobalTimeFilter left, GlobalTimeFilter right,
