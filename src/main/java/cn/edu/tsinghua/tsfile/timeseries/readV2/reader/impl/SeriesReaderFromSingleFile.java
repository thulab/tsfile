...
public abstract class SeriesReaderFromSingleFile implements SeriesReader {
  ...
  public SeriesReaderFromSingleFile(ITsRandomAccessFileReader randomAccessFileReader, Path path)
      throws IOException {
    ...
    this.encodedSeriesChunkDescriptorList =
        new MetadataQuerierByFileImpl(randomAccessFileReader).getSeriesChunkDescriptorList(path);
   ...
   */
  public SeriesReaderFromSingleFile(SeriesChunkLoader seriesChunkLoader,
      List<EncodedSeriesChunkDescriptor> encodedSeriesChunkDescriptorList) {
    ...
    while ((currentReadSeriesChunkIndex + 1) < encodedSeriesChunkDescriptorList.size()) {
      if (!seriesChunkReaderInitialized) {
        EncodedSeriesChunkDescriptor encodedSeriesChunkDescriptor =
            encodedSeriesChunkDescriptorList.get(++currentReadSeriesChunkIndex);
  ...
  protected abstract void initSeriesChunkReader(
      EncodedSeriesChunkDescriptor encodedSeriesChunkDescriptor) throws IOException;

  protected abstract boolean seriesChunkSatisfied(
      EncodedSeriesChunkDescriptor encodedSeriesChunkDescriptor);
