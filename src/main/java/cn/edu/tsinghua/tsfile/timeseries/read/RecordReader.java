...
public class RecordReader {
  ...
  public RecordReader(ITsRandomAccessFileReader raf, List<RowGroupMetaData> rowGroupMetaDataList)
      throws IOException {
  ...
  public DynamicOneColumnData getValueInOneColumn(DynamicOneColumnData res, int fetchSize,
    ...
    List<RowGroupReader> rowGroupReaderList =
        fileReader.getRowGroupReaderListByDeltaObject(deltaObjectUID);
    int i = 0;
    if (res != null) {
   ...
   * @throws IOException TsFile read error
   */
  public DynamicOneColumnData getValueInOneColumnWithoutException(DynamicOneColumnData res,
      int fetchSize, String deltaObjectUID, String measurementUID) throws IOException {
    try {
      checkSeriesByHadoop(deltaObjectUID, measurementUID);
    } catch (IOException ex) {
      if (res == null)
        res = new DynamicOneColumnData();
      res.dataType = fileReader.getRowGroupReaderListByDeltaObject(deltaObjectUID).get(0)
          .getDataTypeBySeriesName(measurementUID);
      return res;
    }
    List<RowGroupReader> rowGroupReaderList =
        fileReader.getRowGroupReaderListByDeltaObjectByHadoop(deltaObjectUID);
    int i = 0;
    if (res != null) {
      i = res.getRowGroupIndex();
    }
  ...
  /**
   * Read one path without filter from one specific <code>RowGroupReader</code> according to the
   * indexList。
   * 
   * @param res result
   * @param fetchSize fetch size
   ...
   */
  public DynamicOneColumnData getValueInOneColumn(DynamicOneColumnData res, int fetchSize,
      String deltaObjectUID, String measurementId, ArrayList<Integer> idxes) throws IOException {
  ...
  public DynamicOneColumnData getValuesUseFilter(DynamicOneColumnData res, int fetchSize,
    ...
    return getValuesUseFilter(res, fetchSize, deltaObjectUID, measurementUID, null, null,
        valueFilter);
  }

  public DynamicOneColumnData getValuesUseFilter(DynamicOneColumnData res, int fetchSize,
      String deltaObjectUID, String measurementId, SingleSeriesFilterExpression timeFilter,
    ...
    for (; i < rowGroupReaderList.size(); i++) {
      ...
      res = getValuesUseFilter(res, fetchSize, rowGroupReader, measurementId, timeFilter,
  ...
  private DynamicOneColumnData getValuesUseFilter(DynamicOneColumnData res, int fetchSize,
    ...
    res = rowGroupReader.getValueReaders().get(measurementId).readOneColumnUseFilter(res, fetchSize,
        timeFilter, freqFilter, valueFilter);
    return res;
  }

  public DynamicOneColumnData getValuesUseTimestamps(String deltaObjectUID, String measurementId,
    ...
    for (int i = 0; i < rowGroupReaderList.size(); i++) {
      ...
      if (i == 0) {
        ...
        DynamicOneColumnData tmpRes =
            getValuesUseTimestamps(rowGroupReader, measurementId, timestamps);
  ...
  public DynamicOneColumnData getValuesUseTimestamps(String deltaObjectUID, String measurementId,
      long[] timeRet, ArrayList<Integer> idxs) throws IOException {
    checkSeries(deltaObjectUID, measurementId);
    DynamicOneColumnData res = null;
    List<RowGroupReader> rowGroupReaderList = fileReader.getRowGroupReaderList();
    ...
    for (int i = 0; i < idxs.size(); i++) {
      ...
      if (!init) {
        ...
        DynamicOneColumnData tmpRes =
            getValuesUseTimestamps(rowGroupReader, measurementId, timeRet);
        res.mergeRecord(tmpRes);
      }
    }
    return res;
  ...
  private DynamicOneColumnData getValuesUseTimestamps(RowGroupReader rowGroupReader,
      String measurementId, long[] timeRet) throws IOException {
  ...
  public FilterSeries<?> getColumnByMeasurementName(String deltaObject, String measurement)
    ...
    if (type == TSDataType.INT32) {
      ...
      return FilterFactory.longFilterSeries(deltaObject, measurement,
          FilterSeriesType.VALUE_FILTER);
    } else if (type == TSDataType.FLOAT) {
      return FilterFactory.floatFilterSeries(deltaObject, measurement,
          FilterSeriesType.VALUE_FILTER);
    } else if (type == TSDataType.DOUBLE) {
      return FilterFactory.doubleFilterSeries(deltaObject, measurement,
          FilterSeriesType.VALUE_FILTER);
    } else if (type == TSDataType.BOOLEAN) {
      return FilterFactory.booleanFilterSeries(deltaObject, measurement,
          FilterSeriesType.VALUE_FILTER);
    } else if (type == TSDataType.ENUMS || type == TSDataType.TEXT) {
      return FilterFactory.stringFilterSeries(deltaObject, measurement,
          FilterSeriesType.VALUE_FILTER);
    } else {
  ...
  private void checkSeries(String deltaObject, String measurement) throws IOException {
    ...
    if (!fileReader.containsDeltaObj(deltaObject)
        || !fileReader.getFileMetaData().containsMeasurement(measurement)) {
      throw new IOException(
          "Series " + deltaObject + "#" + measurement + " does not exist in the current file.");
    }
  }
  ...
  private void checkSeriesByHadoop(String deltaObject, String measurement) throws IOException {
    if (seriesSchemaMap == null) {
      ...
      Map<String, ArrayList<SeriesSchema>> seriesSchemaListMap =
          getAllSeriesSchemasGroupByDeltaObject();
    ...
    throw new IOException(
        "Series is not exist in current file: " + deltaObject + "#" + measurement);
  }
