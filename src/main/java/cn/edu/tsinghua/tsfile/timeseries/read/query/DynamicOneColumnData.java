...
/**
 * DynamicOneColumnData is a self-defined data structure which is optimized for different type of
...
public class DynamicOneColumnData {
  ...
  private void rangeCheck(int idx) {
    if (idx < 0) {
      throw new IndexOutOfBoundsException(
          "DynamicOneColumnData value range check, Index is negative: " + idx);
    }
    if (idx >= valueLength) {
      throw new IndexOutOfBoundsException(
          "DynamicOneColumnData value range check, Index : " + idx + ". Length : " + valueLength);
    }
  }
  ...
  private void rangeCheckForTime(int idx) {
    if (idx < 0) {
      throw new IndexOutOfBoundsException(
          "DynamicOneColumnData time range check, Index is negative: " + idx);
    }
    if (idx >= timeLength) {
      throw new IndexOutOfBoundsException(
          "DynamicOneColumnData time range check, Index : " + idx + ". Length : " + timeLength);
    }
  }
  ...
  private void rangeCheckForEmptyTime(int idx) {
    if (idx < 0) {
      throw new IndexOutOfBoundsException(
          "DynamicOneColumnData empty time range check, Index is negative: " + idx);
    }
    if (idx >= emptyTimeLength) {
      throw new IndexOutOfBoundsException("DynamicOneColumnData empty time range check, Index : "
          + idx + ". Length : " + emptyTimeLength);
    }
  }
   ...
   * @return the new DynamicOneColumnData whose data is equals to position startPos and position
   *         endPos
   */
  ...
  public void plusRowGroupIndexAndInitPageOffset() {
    ...
    this.rowGroupIndex++;
    // RowGroupIndex's change means that The pageOffset should be updateTo the value in next
    // RowGroup.
