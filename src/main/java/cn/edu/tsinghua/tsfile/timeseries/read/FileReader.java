...
/**
 * This class is used to read <code>TSFileMetaData</code> and construct file level reader which
 * contains the information of <code>RowGroupReader</code>.
 *
 * @author Jinrui Zhang
 */
public class FileReader {
  ...
  /**
   * If the file has many rowgroups and series, the storage of <code>fileMetaData</code> may be
   * large.
   */
  ...
  /**
   * Lock when initializing RowGroupReaders so that the same deltaObj will not be initialized more
   * than once.
   */
  ...
  /**
   * //TODO verify rightness Used for IoTDB compatibility
   *
   * @param rowGroupMetadataList
   */
  ...
  /**
   * Get all readers that access every RowGroup belonging to deltaObjectUID within this file. This
   * method will try to init the readers if they are uninitialized(non-exist).
   *
   * @param deltaObjectUID name of the desired deltaObject
   * @return A list of RowGroupReaders specified by deltaObjectUID or NULL if such deltaObject
  ...
  public List<RowGroupReader> getRowGroupReaderListByDeltaObjectByHadoop(String deltaObjectUID)
      throws IOException {
    return this.rowGroupReaderMap.get(deltaObjectUID);
  }
  ...
  /*
   * The below methods can be used to init RowGroupReaders of a given deltaObj in different ways, in
   * case of another refactoring. Current method is based on TsDeltaObject.
   */
  ...
  private void initRowGroupReaders(TsDeltaObject deltaObj) throws IOException {
    ...
    blockMeta.convertToTSF(ReadWriteThriftFormatUtils.readRowGroupBlockMetaData(
        this.randomAccessFileReader, deltaObj.offset, deltaObj.metadataBlockSize));
    initRowGroupReaders(blockMeta.getRowGroups());
  }

  /**
   * Core method, construct RowGroupReader for every RowGroup in given list, thread-unsafe. The
   * caller should avoid adding duplicate readers.
   *
   * @param groupList
   */
  ...
  /**
   * Add a deltaObj by its name to the tail of the LRU list. If the deltaObj already exists, remove
   * it. When adding a new item, check if the volume exceeds, if so, remove the head of list and
   * responding RowGroupReaders.
   *
   * @param deltaObjUID
   */
  ...
  /**
   * This method prefetch metadata of a DeltaObject for methods like checkSeries, if the DeltaObject
   * is not in memory.
   *
   * @param deltaObjUID
   */
  ...
  public List<RowGroupMetaData> getSortedRowGroupMetaDataList() throws IOException {
    ...
    Comparator<RowGroupMetaData> comparator = new Comparator<RowGroupMetaData>() {
      ...
      public int compare(RowGroupMetaData o1, RowGroupMetaData o2) {
        ...
        return Long.signum(o1.getMetaDatas().get(0).getProperties().getFileOffset()
