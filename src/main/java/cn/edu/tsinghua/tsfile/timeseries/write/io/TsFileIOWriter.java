...
public class TsFileIOWriter {
  ...
  public void startRowGroup(long recordCount, String deltaObjectId) {
    ...
    currentRowGroupMetaData =
        new RowGroupMetaData(deltaObjectId, recordCount, 0, new ArrayList<>(), "");
  ...
  public void endFile(FileSchema schema) throws IOException {
    ...
    LinkedHashMap<String, TsRowGroupBlockMetaData> tsRowGroupBlockMetaDataMap =
        new LinkedHashMap<>();
    ...
    while (iterator.hasNext()) {
      ...
      ReadWriteThriftFormatUtils.writeRowGroupBlockMetadata(
          currentTsRowGroupBlockMetaData.convertToThrift(), out.getOutputStream());
      offset = out.getPos();
      TsDeltaObject tsDeltaObject =
          new TsDeltaObject(offsetIndex, (int) (offset - offsetIndex), startTime, endTime);
      tsDeltaObjectMap.put(currentDeltaObject, tsDeltaObject);
    }

    TsFileMetaData tsFileMetaData =
        new TsFileMetaData(tsDeltaObjectMap, timeSeriesList, TSFileConfig.currentVersion);
