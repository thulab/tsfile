...
public abstract class CrossQueryTimeGenerator {
  ...
  protected ArrayList<Integer> idxCount; // represent the dfsCnt and the sum node number of its'
                                         // subtree
  ...
  public CrossQueryTimeGenerator(SingleSeriesFilterExpression timeFilter,
      SingleSeriesFilterExpression freqFilter, FilterExpression valueFilter, int fetchSize) {
  ...
  public long[] generateTimes() throws ProcessorException, IOException {
    ...
    while (cnt < fetchSize) {
      ...
      if ((timeFilter == null)
          || (timeFilter != null && timeFilterVisitor.satisfyObject(v, timeFilter))) {
        res[cnt] = v;
        cnt++;
      }
    }
  ...
  private long calculateOneTime(FilterExpression valueFilter)
  ...
  public DynamicOneColumnData getMoreRecordForOneCol(int idx,
  ...
  /**
   * valueFilterNumber parameter is mainly used for IoTDB. Because of the exist of
   * <code>RecordReaderCache</code>, we must know the occur position of the SingleSeriesFilter in
   * CrossSeriesFilterExpression.
   *
