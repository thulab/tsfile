...
public class NodeConstructorForSingleFileImpl extends NodeConstructor {
  ...
  public NodeConstructorForSingleFileImpl(MetadataQuerier metadataQuerier,
  ...
  public SeriesReader generateSeriesReader(SeriesFilter<?> seriesFilter) throws IOException {
    List<EncodedSeriesChunkDescriptor> encodedSeriesChunkDescriptorList =
        metadataQuerier.getSeriesChunkDescriptorList(seriesFilter.getSeriesPath());
    return new SeriesReaderFromSingleFileWithFilterImpl(seriesChunkLoader,
        encodedSeriesChunkDescriptorList, seriesFilter.getFilter());
  }
}
