...
public class SeriesChunkLoaderImpl implements SeriesChunkLoader {
  ...
  public MemSeriesChunk getMemSeriesChunk(EncodedSeriesChunkDescriptor encodedSeriesChunkDescriptor)
      throws IOException {
    try {
      return new MemSeriesChunk(encodedSeriesChunkDescriptor,
          new ByteArrayInputStream(seriesChunkBytesCache.get(encodedSeriesChunkDescriptor)));
    } catch (CacheException e) {
      throw new IOException(e);
    }
  }

  private byte[] load(EncodedSeriesChunkDescriptor encodedSeriesChunkDescriptor)
      throws IOException {
    ...
    if (readLength != seriesChunkLength) {
      throw new IOException("length of seriesChunk read from file is not right. Expected:"
          + seriesChunkLength + ". Actual: " + readLength);
    }
    return buf;
  }
}
