  ...
  public QueryDataSet execute(QueryExpression queryExpression) throws IOException {
    if (queryExpression.hasQueryFilter()) {
      try {
        ...
        QueryFilter regularQueryFilter = QueryFilterOptimizer.getInstance()
            .convertGlobalTimeFilter(queryFilter, queryExpression.getSelectedSeries());
        queryExpression.setQueryFilter(regularQueryFilter);
        if (regularQueryFilter instanceof GlobalTimeFilter) {
          return new QueryWithGlobalTimeFilterExecutorImpl(seriesChunkLoader, metadataQuerier)
              .execute(queryExpression);
        } else {
          return new QueryWithQueryFilterExecutorImpl(seriesChunkLoader, metadataQuerier)
              .execute(queryExpression);
        }
      ...
      return new QueryWithoutFilterExecutorImpl(seriesChunkLoader, metadataQuerier)
          .execute(queryExpression);
    }
  }
}
