package cn.edu.tsinghua.ts2file.timesegment.write;
...
import cn.edu.tsinghua.ts2file.timesegment.write.segment.IRowGroupWriter;
import cn.edu.tsinghua.ts2file.timesegment.write.segment.RowGroupWriterImpl;
import java.io.File;
...
/**
 * TwFileWriter is the entrance for writing processing. It receives a record and send it to
 ...
 *
 */
public class Ts2FileWriter {
  private static final Logger LOG = LoggerFactory.getLogger(Ts2FileWriter.class);
  ...
  public Ts2FileWriter(File file) throws WriteProcessException, IOException {
    this(new TsFileIOWriter(file), new FileSchema(), TSFileDescriptor.getInstance().getConfig());
  }

  public Ts2FileWriter(File file, FileSchema schema) throws WriteProcessException, IOException {
    this(new TsFileIOWriter(file), schema, TSFileDescriptor.getInstance().getConfig());
  }

  public Ts2FileWriter(File file, TSFileConfig conf) throws WriteProcessException, IOException {
    this(new TsFileIOWriter(file), new FileSchema(), conf);
  }

  public Ts2FileWriter(File file, FileSchema schema, TSFileConfig conf)
      throws WriteProcessException, IOException {
    this(new TsFileIOWriter(file), schema, conf);
  }

  public Ts2FileWriter(ITsRandomAccessFileWriter output) throws WriteProcessException, IOException {
    this(new TsFileIOWriter(output), new FileSchema(), TSFileDescriptor.getInstance().getConfig());
  }

  public Ts2FileWriter(ITsRandomAccessFileWriter output, FileSchema schema)
      throws WriteProcessException, IOException {
    this(new TsFileIOWriter(output), schema, TSFileDescriptor.getInstance().getConfig());
  }

  public Ts2FileWriter(ITsRandomAccessFileWriter output, TSFileConfig conf)
      throws WriteProcessException, IOException {
    this(new TsFileIOWriter(output), new FileSchema(), conf);
  }

  public Ts2FileWriter(ITsRandomAccessFileWriter output, FileSchema schema, TSFileConfig conf)
      throws WriteProcessException, IOException {
    this(new TsFileIOWriter(output), schema, conf);
  }

  protected Ts2FileWriter(TsFileIOWriter twfileWriter, FileSchema schema, TSFileConfig conf)
      throws WriteProcessException {
    this.deltaFileWriter = twfileWriter;
    this.schema = schema;
   ...
   */
  protected boolean checkIsTimeWindowExist(Ts2Record record) throws WriteProcessException {
		...
		for (SegmentDataPoint dp : record.dataPointList) {
			String measurementId = dp.getMeasurementId();
			if (schemaDescriptorMap.containsKey(measurementId))
			  groupWriter.addWindowWriter(schemaDescriptorMap.get(measurementId), pageSize);
			else
   ...
   * @return true -size of twfile or metadata reaches the threshold.
   * false - otherwise
   */
  public boolean write(Ts2Record record) throws IOException, WriteProcessException {
    if (checkIsTimeWindowExist(record)) {
      groupWriters.get(record.deltaObjectId).write(record.startTime, record.endTime, record.dataPointList);
      ++recordCount;
  ...
  /**
   * flush the data in all window writers and their page writers to outputStream.
   * 
