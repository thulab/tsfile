package cn.edu.tsinghua.ts2file.timesegment.basis;

import cn.edu.tsinghua.ts2file.timesegment.utils.RecordUtils;
...
import cn.edu.tsinghua.ts2file.timesegment.read.query.QueryTs2File;
 ...
 * @author qmm
 */
public class Ts2File {

  private static final int WRITE = 0;
  private static final int READ = 1;
  private QueryTs2File queryTs2File;
  private int status;
  private Ts2FileWriter writer;
   ...
   */
  public Ts2File(File file, JSONObject schemaJson) throws IOException, WriteProcessException {
   ...
   */
  public Ts2File(File file, FileSchema schema) throws IOException, WriteProcessException {
    this(schema);
    writer = new Ts2FileWriter(file, fileSchema, TSFileDescriptor.getInstance().getConfig());
  }
   ...
   *           exception in write process
   */
  public Ts2File(ITsRandomAccessFileWriter output, JSONObject schemaJson)
      throws IOException, WriteProcessException {
    this(new FileSchema(schemaJson));
    TSFileConfig conf = TSFileDescriptor.getInstance().getConfig();
    writer = new Ts2FileWriter(output, fileSchema, conf);
  }
   ...
   *           error occurs when writing
   */
  public Ts2File(ITsRandomAccessFileWriter output, FileSchema schema)
      throws IOException, WriteProcessException {
    this(schema);
    writer = new Ts2FileWriter(output, fileSchema, TSFileDescriptor.getInstance().getConfig());
  }

  private Ts2File(FileSchema schema) {
    fileSchema = schema;
    TSFileConfig conf = TSFileDescriptor.getInstance().getConfig();
   ...
   *           cannot read TsFile
   */
  public Ts2File(ITsRandomAccessFileReader raf) throws IOException {
    this.status = READ;
    queryTs2File = new QueryTs2File(raf);
    // recordReader = queryEngine.recordReader;
  }

  /**
   * write a line into TwFile <br>
  ...
  public void writeLine(String line) throws IOException, WriteProcessException {
    checkStatus(WRITE);
    Ts2Record record = RecordUtils.parseSimpleTupleRecord(line, fileSchema);
    writer.write(record);
  }
  ...
  /**
   * write a TWRecord into TsFile.
   *
   * @param ts2Record
   *          a line of data in form of {@linkplain Ts2Record}
   * @throws IOException
   *           thrown if write process meats IOException like the output stream is closed
   *           abnormally.
   * @throws WriteProcessException
   *           thrown if given data is not matched to fileSchema
   */
  public void writeRecord(Ts2Record ts2Record) throws IOException, WriteProcessException {
    checkStatus(WRITE);
    writer.write(ts2Record);
  }
  ...
  public QueryDataSet query(List<Path> paths, FilterExpression timeFilter,
    ...
    return queryTs2File.query(paths, timeFilter, null, valueFilter);
  }
  ...
  public QueryDataSet query(List<Path> paths, FilterExpression timeFilter,
    ...
    checkStatus(READ);
    return queryTs2File.query(paths, timeFilter, null, valueFilter, params);
  }
  ...
  public Map<String, ArrayList<SeriesSchema>> getAllColumns() throws IOException {
    checkStatus(READ);
    return queryTs2File.getAllSeriesSchemasGroupByDeltaObject();
  }
  ...
  public Map<String, Integer> getDeltaObjectRowGroupCount() throws IOException {
    checkStatus(READ);
    return queryTs2File.getDeltaObjectRowGroupCount();
  }
  ...
  public Map<String, String> getDeltaObjectTypes() throws IOException {
    checkStatus(READ);
    return queryTs2File.getDeltaObjectTypes();
  }
  ...
  public boolean pathExist(Path path) throws IOException {
    checkStatus(READ);
    return queryTs2File.pathExist(path);
  }
  ...
  public ArrayList<String> getAllDeltaObject() throws IOException {
    checkStatus(READ);
    return queryTs2File.getAllDeltaObject();
  }
  ...
  public List<SeriesSchema> getAllSeries() throws IOException {
    checkStatus(READ);
    return queryTs2File.getAllSeriesSchema();
  }
  ...
  public ArrayList<Long> getRowGroupPosList() throws IOException {
    checkStatus(READ);
    return queryTs2File.getRowGroupPosList();
  }

  public ArrayList<Integer> calSpecificRowGroupByPartition(long start, long end)
      throws IOException {
    checkStatus(READ);
    return queryTs2File.calSpecificRowGroupByPartition(start, end);
  }

  public ArrayList<String> getAllDeltaObjectUIDByPartition(long start, long end)
      throws IOException {
    checkStatus(READ);
    return queryTs2File.getAllDeltaObjectUIDByPartition(start, end);
  }

  public Map<String, String> getProps() {
    return queryTs2File.getProps();
  }
  ...
  public String getProp(String key) {
    return queryTs2File.getProp(key);
  }
