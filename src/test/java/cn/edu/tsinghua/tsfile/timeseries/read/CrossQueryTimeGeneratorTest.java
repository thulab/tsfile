...
public class CrossQueryTimeGeneratorTest {
  ...
  public void singleValueFilterSplitTest() throws IOException, ProcessorException {
    SingleSeriesFilterExpression timeFilter =
        FilterFactory.gtEq(FilterFactory.timeFilterSeries(), 60L, true);
    SingleSeriesFilterExpression s1Filter = FilterFactory
        .ltEq(FilterFactory.intFilterSeries(d1, s1, FilterSeriesType.VALUE_FILTER), 100, true);
    SingleSeriesFilterExpression s2Filter = FilterFactory
        .ltEq(FilterFactory.intFilterSeries(d1, s2, FilterSeriesType.VALUE_FILTER), 200, true);
    SingleSeriesFilterExpression s3Filter = FilterFactory
        .ltEq(FilterFactory.intFilterSeries(d1, s3, FilterSeriesType.VALUE_FILTER), 200, true);
    SingleSeriesFilterExpression s4Filter = FilterFactory
        .ltEq(FilterFactory.intFilterSeries(d1, s4, FilterSeriesType.VALUE_FILTER), 200, true);
    ...
    CrossQueryTimeGenerator generator =
        new CrossQueryTimeGenerator(timeFilter, null, crossFilter3, 1000) {
          @Override
