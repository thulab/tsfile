...
public class IntervalTimeVisitorTest {
  ...
  public void test() {
    ...
    NotEq<Long> noteq = FilterFactory.noteq(FilterFactory.longFilterSeries(deltaObjectUID,
        measurementUID, FilterSeriesType.VALUE_FILTER), 45L);
    ...
    GtEq<Long> gteq = FilterFactory.gtEq(FilterFactory.longFilterSeries(deltaObjectUID,
        measurementUID, FilterSeriesType.VALUE_FILTER), 45L, true);
    assertTrue(filter.satisfy(gteq, 10L, 50L));

    LtEq<Long> left = FilterFactory.ltEq(FilterFactory.longFilterSeries(deltaObjectUID,
        measurementUID, FilterSeriesType.VALUE_FILTER), 55L, true);


    GtEq<Long> right = FilterFactory.gtEq(FilterFactory.longFilterSeries(deltaObjectUID,
        measurementUID, FilterSeriesType.VALUE_FILTER), 35L, true);
