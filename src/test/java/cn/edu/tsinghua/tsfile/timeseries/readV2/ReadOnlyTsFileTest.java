...
public class ReadOnlyTsFileTest {
  ...
  public void queryTest() throws IOException {
    ...
    QueryFilter queryFilter =
        QueryFilterFactory.or(QueryFilterFactory.and(new SeriesFilter<>(new Path("d1.s1"), filter),
            new SeriesFilter<>(new Path("d1.s4"), filter2)), new GlobalTimeFilter(filter3));

    QueryExpression queryExpression = QueryExpression.create().addSelectedPath(new Path("d1.s1"))
        .addSelectedPath(new Path("d1.s4")).setQueryFilter(queryFilter);
    ...
    queryExpression = QueryExpression.create().addSelectedPath(new Path("d1.s1"))
        .addSelectedPath(new Path("d1.s4")).setQueryFilter(new GlobalTimeFilter(filter3));
    queryDataSet = tsFile.query(queryExpression);
    aimedTimestamp = 1480562618000L;
    count = 0;
    while (queryDataSet.hasNext()) {
