...
public class QueryExecutorTest {
  ...
  public void before() throws InterruptedException, WriteProcessException, IOException {
    ...
    queryExecutorWithQueryFilter =
        new QueryWithQueryFilterExecutorImpl(seriesChunkLoader, metadataQuerierByFile);
  }
  ...
  public void query1() throws IOException {
    ...
    // Filter<Integer> filter3 = FilterFactory.and(TimeFilter.gtEq(1480562618000L),
    // TimeFilter.ltEq(1480562618100L));

    QueryFilter queryFilter = QueryFilterFactory.and(new SeriesFilter<>(new Path("d1.s1"), filter),
        new SeriesFilter<>(new Path("d1.s4"), filter2));

    // QueryFilter queryFilter = new SeriesFilter<>(new SeriesDescriptor(new Path("d1.s1"),
    // TSDataType.INT32), filter);
    ...
    QueryExpression queryExpression = QueryExpression.create().addSelectedPath(new Path("d1.s1"))
        ...
        .addSelectedPath(new Path("d1.s5")).setQueryFilter(queryFilter);
  ...
  public void queryWithoutFilter() throws IOException {
    QueryExecutor queryExecutor =
        new QueryWithoutFilterExecutorImpl(seriesChunkLoader, metadataQuerierByFile);

    QueryExpression queryExpression = QueryExpression.create().addSelectedPath(new Path("d1.s1"))
        .addSelectedPath(new Path("d1.s2")).addSelectedPath(new Path("d1.s2"))
  ...
  public void queryWithGlobalTimeFilter() throws IOException {
    QueryExecutor queryExecutor =
        new QueryWithGlobalTimeFilterExecutorImpl(seriesChunkLoader, metadataQuerierByFile);

    QueryFilter queryFilter = new GlobalTimeFilter(
