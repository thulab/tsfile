...
public class GenerateBigDataCSV {
  ...
  // s4:log
  private static int[][] brokenLineConfigs =
      {{1000, 1, 100}, {200000, 5, 200000}, {10000, 2, 50000}};
  ...
  private static void getNextRecordToFile(long timestamp, long index, FileWriter fw)
      throws IOException {
    for (int i = 0; i < deviceCount; i++) {
      ...
      if (sensorSet.contains("s2")) {// s2:square wave, float
        ...
        freqWave[i] =
            (hasWrittenFreq[i] == 1)
                ? (float) (squareAmplitude[i] / 2
                    * Math.sin(sinAbnormalConfigs[i][0] * 2 * Math.PI * index))
                : 0;
  ...
  private static long generateSinGlitch(long t, int i) {
    if (r.nextDouble() < glitchProbability[i] && width[i] == -1) {
      startAbTime[i] = t;
      base[i] = (long) (maxMinVertical[i][0] + sinConfigs[i][1]
          + sinConfigs[i][1] * Math.sin(sinConfigs[i][0] * t));
      width[i] =
          r.nextInt(maxMinHorizontal[i][0] - maxMinHorizontal[i][1]) + maxMinHorizontal[i][1];

      if (width[i] < 2)
        width[i] = 2;
      mid[i] = r.nextInt(width[i] - 1) + 1;
      upPeek[i] = maxMinVertical[i][1] + r.nextInt(maxMinVertical[i][0] - maxMinVertical[i][1]);
      downPeek[i] = maxMinVertical[i][1] + r.nextInt(maxMinVertical[i][0] - maxMinVertical[i][1]);
      return base[i];
    } else {
      if (width[i] != -1) {
        long value;
        ...
        if (t - startAbTime[i] <= mid[i]) {
          ...
          value = (long) (base[i] + upPeek[i]
              - ((double) t - mid[i] - startAbTime[i]) / (width[i] - mid[i]) * downPeek[i]);
        }
        ...
        return (long) (maxMinVertical[i][0] + sinConfigs[i][1]
            + sinConfigs[i][1] * Math.sin(sinConfigs[i][0] * t));
      }
    }
  }
  ...
  public static void main(String[] args) throws IOException, InterruptedException {
    if (args.length < 4) {
      ...
      System.err.println(
          "input format: <csvFilePath> <schemaOutputFilePath> <lineCount> <sensorName> [<sensorName>...]");
      return;
    }
