...
public class QueryFilterOptimizerTest {
  ...
  public void testTimeOnly() {
    try {
      ...
      QueryFilter queryFilter2 =
          QueryFilterFactory.or(
              QueryFilterFactory.and(new GlobalTimeFilter(TimeFilter.lt(50L)),
                  new GlobalTimeFilter(TimeFilter.gt(10L))),
  ...
  public void testSeriesOnly() {
    try {
      Filter<Long> filter1 = FilterFactory
          .and(FilterFactory.or(ValueFilter.gt(100L), ValueFilter.lt(50L)), TimeFilter.gt(1400L));
      SeriesFilter<Long> seriesFilter1 = new SeriesFilter<>(new Path("d2.s1"), filter1);

      Filter<Float> filter2 = FilterFactory.and(
          FilterFactory.or(ValueFilter.gt(100.5f), ValueFilter.lt(50.6f)), TimeFilter.gt(1400L));
      SeriesFilter<Float> seriesFilter2 = new SeriesFilter<>(new Path("d1.s2"), filter2);

      Filter<Double> filter3 = FilterFactory
          .or(FilterFactory.or(ValueFilter.gt(100.5), ValueFilter.lt(50.6)), TimeFilter.gt(1400L));
      SeriesFilter<Double> seriesFilter3 = new SeriesFilter<>(new Path("d2.s2"), filter3);

      QueryFilter queryFilter = QueryFilterFactory
          .and(QueryFilterFactory.or(seriesFilter1, seriesFilter2), seriesFilter3);
  ...
  public void testOneTimeAndSeries() {
    ...
    QueryFilter queryFilter = QueryFilterFactory
        .and(QueryFilterFactory.or(seriesFilter1, seriesFilter2), globalTimeFilter);
    QueryFilterPrinter.print(queryFilter);
    try {
      String rightRet =
          "[[d2.s1:((value > 100 || value < 50) && time < 14001234)] || [d1.s2:((value > 100.5 || value < 50.6) && time < 14001234)]]";
      QueryFilter regularFilter =
          queryFilterOptimizer.convertGlobalTimeFilter(queryFilter, selectedSeries);
  ...
  public void testOneTimeOrSeries() {
    ...
    QueryFilter globalTimeFilter = new GlobalTimeFilter(timeFilter);
    QueryFilter queryFilter = QueryFilterFactory
        .or(QueryFilterFactory.or(seriesFilter1, seriesFilter2), globalTimeFilter);
    QueryFilterPrinter.print(queryFilter);
  ...
  public void testTwoTimeCombine() {
    ...
    QueryFilter queryFilter =
        QueryFilterFactory.or(QueryFilterFactory.or(seriesFilter1, seriesFilter2),
            QueryFilterFactory.and(globalTimeFilter1, globalTimeFilter2));

    try {
      String rightRet =
          "[[[[[d1.s1:(time < 14001234 && time > 14001000)] || [d2.s1:(time < 14001234 && time > 14001000)]] "
    ...
    QueryFilter queryFilter2 =
        QueryFilterFactory.and(QueryFilterFactory.or(seriesFilter1, seriesFilter2),
            QueryFilterFactory.and(globalTimeFilter1, globalTimeFilter2));
    ...
    try {
      ...
      QueryFilter regularFilter2 =
          queryFilterOptimizer.convertGlobalTimeFilter(queryFilter2, selectedSeries);
    ...
    try {
      QueryFilter regularFilter3 =
          queryFilterOptimizer.convertGlobalTimeFilter(queryFilter3, selectedSeries);
